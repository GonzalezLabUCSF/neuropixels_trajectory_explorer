...
% tv = cast(rescale(permute(niftiread(fullfile(atlas_path,'atlas.img')),[2,3,1]),0,255),'uint8'); % casting to unsigned int 8
% %av = permute(niftiread(fullfile(atlas_path,'WHS_SD_rat_atlas_v4.nii.gz')),[2,3,1]);
% av = permute(niftiread(fullfile(atlas_path,'atlas.img')),[2,3,1]); % read the atlas 



tv = single(cast(rescale(permute(niftiread(fullfile(atlas_path,'atlas.img')),[2,3,1]),0,255),'uint8'));
[x_original, y_original, z_original] = meshgrid(1:size(tv, 2), 1:size(tv, 3), 1:size(tv, 1));
[x_desired, y_desired, z_desired] = meshgrid(linspace(1, size(tv, 2), 512), ...
                                             linspace(1, size(tv, 3), 1024), ...
                                             linspace(1, size(tv, 1), 512));
mri_data_upscaled = interp3(x_original, y_original, z_original, tv, ...
                             x_desired, y_desired, z_desired, 'linear');
tv = cast(mri_data_upscaled,'uint8');
av = cast (mri_data_upscaled,'uint16');
...
st = table; 
st(st_raw.index(2:end-1),:) = st_raw(2:end-1,:);
...
brain_volume = ...
    bwmorph3(bwmorph3(av(1:slice_spacing:end, ...
    1:slice_spacing:end,1:slice_spacing:end)>50,'majority'),'majority');
...
gui_data.probe_angle = [0;45]; % Probe angles in ML/DV
