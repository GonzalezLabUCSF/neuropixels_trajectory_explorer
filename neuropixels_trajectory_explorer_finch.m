...
addpath('C:\Users\Gonzalez Lab\Lab Dropbox\Code\NPIX\npy-matlab-master\npy-matlab');
...
roi =  single(cast(rescale(permute(niftiread(fullfile(atlas_path,'testing_segment.nii.gz')),[2,3,1]),0,255),'uint8'));
...
st_t =  load_structure_tree([atlas_path filesep 'zebra_finch_structure_tree_safe_2017.csv' ]);
...
ccf_color_hex = st_t.color_hex_triplet;
...
bregma_lambda_distance_avg = 2.20; % Currently approximation

% (translation values from our bregma estimate: AP/ML from Paxinos, DV from
% rough MRI estimate)
bregma_ccf = [125,120,183]; % [ML,AP,DV]
...
ap_rotation = 0; % tilt the CCF 5 degrees nose-up
ccf_rotation_tform = ...
    [1 0 0 0; ...
