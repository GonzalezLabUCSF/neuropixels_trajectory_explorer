...
function neuropixels_trajectory_explorer_finch
...
%st =  load_structure_tree([atlas_path filesep 'zebra_finch_structure_tree_safe_2017.csv']);

% filename = fullfile(atlas_path,'test_label.label');
% startRow = 16;
% formatSpec = '%5f%6f%5f%5f%9f%3f%3f%s%s%[^\n\r]';
% fileID = fopen(filename,'r');
% %dataArray = textscan(fileID, formatSpec, 'Delimiter', '', 'WhiteSpace', '', 'TextType', 'string', 'HeaderLines' ,startRow-1, 'ReturnOnError', false, 'EndOfLine', '\r\n');
% dataArray = textscan(fileID,formatSpec);
% dataArray{8} = strtrim(dataArray{8});
% fclose(fileID);
% st_raw = table(dataArray{1:end-1}, 'VariableNames', {'index','R','G','B','A','VIS','MSH','color_hex_triplet','safe_name'});
% st = table; 
% st(st_raw.index(2:end),:) = st_raw(2:end,:);
% clearvars filename startRow formatSpec fileID dataArray ans;

%ccf_cmap = [st_t.R,st_t.G,st_t.B]/255;
...
% 
% ccf_color_hex = st.color_hex_triplet;
% ccf_cmap = hex2rgb(ccf_color_hex);
...
uimenu(scaling_menu,'Text','Set Sinus-HVC distance','MenuSelectedFcn',{@set_bregma_lambda_distance,probe_atlas_gui});
...
gui_data.handles.slice_plot = surface(axes_atlas,'EdgeColor','flat','Visible','off'); % Slice on 3D atlas
...
if strcmp(gui_data.handles.slice_plot(1).Visible,'on')
    ...
    switch gui_data.handles.slice_volume
        case 'tv'
            ...
            curr_slice(curr_slice < 5) = NaN; % threshold values

            colormap(gui_data.handles.axes_atlas,'copper');
...
bregma_lambda_text = sprintf('Sinus-HVC distance: % .2f mm', ...
...
probe_text_with_scale = {probe_angle_text,probe_insertion_text, ...
    probe_tip_text,bregma_lambda_text, ...
    manipulator_text,recording_text};
...
% new_ap = (insertion_point(2)* gui_data.bregma_lambda_distance_avg)/bregma_lambda_distance;
% disp(new_ap)

% new update for scaling refer 
% https://www.researchgate.net/post/In_stereotaxic_surgery_do_you_have_to_modify_your_target_coordinates_if_your_Bregma-Lambda_distance_is_more_less_than_90_mm
old_ml = gui_data.probe(gui_data.selected_probe).trajectory.XData(1);
old_ap = gui_data.probe(gui_data.selected_probe).trajectory.YData(1);

new_ml = old_ml * bregma_lambda_distance/(gui_data.bregma_lambda_distance_curr);
new_ap = old_ap * bregma_lambda_distance/(gui_data.bregma_lambda_distance_curr);
new_dv = gui_data.probe(gui_data.selected_probe).trajectory.ZData(1)

%disp(gui_data.probe(gui_data.selected_probe).dv)
bird_scale = [new_ml new_ap 1 1];
coords = [old_ml new_ml old_ap new_ap]

%disp(new_ml)
% multiply the identity matrix mst = eye(4); by a separate value x, y and z
% and finaly have an matrix of different values in matlab

% Get the sizing scale of this mouse to the reference atlas
mouse_scale = bregma_lambda_distance/gui_data.bregma_lambda_distance_avg;
%disp(mouse_scale)

% Apply scale to reference transform and set to new transform
mouse_scale_tform = eye(4).*[repmat(mouse_scale,3,1);1];
%mouse_scale_tform = diag(bird_scale);
disp(mouse_scale_tform)
gui_data.ccf_bregma_tform.T = gui_data.ccf_bregma_tform_ref.T*mouse_scale_tform;

%save("mouseform.mat","gui_data.ccf_bregma_tform");

% Update current bregma-lambda distance
gui_data.bregma_lambda_distance_curr = bregma_lambda_distance;

gui_data.probe(gui_data.selected_probe).trajectory.XData(1)= new_ml;
gui_data.probe(gui_data.selected_probe).trajectory.YData(1) = new_ap;
...
old_trajectory_vector = get(gui_data.probe(gui_data.selected_probe).trajectory,{'XData','YData'});
probe_tip_text = sprintf('New Coordinates:       % .2f AP, % .2f ML, % .2f DV', ...
    new_ap,new_ml,new_dv);
%set(gui_data.probe_coordinates_text,'String',probe_tip_text);
%disp(probe_tip_text)
...
% mouse_scale = bregma_lambda_distance/gui_data.bregma_lambda_distance_avg;
% 
% % probe coordinates need to be updated
% update_probe_areas_coordinates(probe_atlas_gui);


%% Control functions

function view_coronal(h,~,probe_atlas_gui)
% Set coronal view
gui_data = guidata(probe_atlas_gui);
...
bregma_lambda_distance_new = ...
    cellfun(@str2num,inputdlg('Sinus-HVC distance (mm):','', ...
...
if nargin<1
    ...
    %fn = fullfile(fileparts(fileparts(p)), 'zebra_structure_tree_safe_2017.csv'); %%%% works for the zebra_structure_tree_safe_2017.csv file
end
%disp(fn)
[~, fnBase] = fileparts(fn);
...
elseif strcmp(mode, '2017')
    ...
    %titles = textscan(fid, repmat('%s', 1, 26), 1, 'delimiter', ','); % works for the zebra_structure_tree_safe_2017.csv file
    titles = cellfun(@(x)x{1}, titles, 'uni', false);
